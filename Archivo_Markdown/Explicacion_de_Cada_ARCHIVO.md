...
## 6. TRABAJO_REALIZADO_EN_ADMIN.md
...
## 7. TRABAJO_REALIZADO_EN_PROFESOR.md
...
## 8. TRABAJO_REALIZADO_EN_ALUMNO.md
...
Para qué sirve: responder de un vistazo "¿qué se hizo en cada fase y qué falta?" sin tener que leer el historial de commits; es el documento que se actualiza al cerrar cada fase (uno de los criterios de aceptación de todos los planes).
...
- **Instrucciones**: prompt (qué pedir) → planes `PLAN_*` (cómo se decidió: Claude, Admin, Profesor, Alumno).
- **Estado**: informes `TRABAJO_REALIZADO_*` (qué hay: Admin, Profesor, Alumno) + `CHANGELOG.md` (historial por fases).
- **Uso**: `COMANDOS_DE_LA_APP.md` (manual completo) → `run_comando.md` (arranque express) → `README.md` (presentación general).
